...
app.post('/api/confirmar-compra', async (req, res) => {
  ...
  const telefono = normalizarTelefono(req.body?.numero);
  ...
  if (telefono.length < 8) {
    return res.status(400).json({ ok: false, message: 'Número de contacto inválido.' });
  }
